...
public class PlasmidsTest extends PurchasableCollectionTest{
	...
	@Test (groups = {"deep"})
	public void addNewPlasmidWithSequence(){
		
		try {
			getPageManager().getAdminPage().showPlasmids();
			
			String plasmidName = "Plasmid with sequence";
			
			String msg = getPageManager().getPlasmidsPage().addNewPlasmidWithSequence(plasmidName);
			
			// Check the title of the page
			 assertEquals(getMessageSource().getMessage("collection.created.msg",new Object[]{"Plasmid"}, Locale.US), msg.trim());
		}catch (Exception e) {
			setLog(e,"addNewPlasmidWithSequence");
			Assert.fail(e.getMessage());
		}
	}
